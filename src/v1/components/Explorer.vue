<template>
    <div class="row margin-t-md">
        <div class="col-sm-12">
            <h2>V1 API explorer</h2>
            <h5>Try out some queries below to get familiar with the first version of the Rets Rabbit API</h5>
        </div>
        <div class="col-sm-12 personal-info margin-t-md">
            <hr>
            <div class="explorer-wrapper">
                <div class="row">
                    <div class="col-sm-10 col-sm-offset-1 col-lg-8 col-lg-offset-2">
                        <h3>Request</h3>
                        <div class="request margin-t-sm" v-cloak>
                            <code>{{ queryUrl }}</code>
                        </div>
                    </div>
                </div>
                <div class="row margin-t-lg">
                    <div class="col-sm-10 col-sm-offset-1 col-lg-8 col-lg-offset-2">
                        <h3>Build Query</h3>
                        <form role="form" class="form-horizontal" @submit.prevent="search">
                            <div class="form-group margin-t-lg">
                                <div class="col-sm-12">
                                    <button @click="addField" class="btn btn-success" type="button">Add Field</button>
                                    <button @click="search" type="submit" v-bind:disabled="!ready || queryString.length < 1 || query.fetching" class="btn btn-primary pull-right">
    ...
    <div class="row">
        <div class="col-sm-10 col-sm-offset-1">
            <h3 class="text-center">Results <span v-show="query.results" v-cloak>:&nbsp;{{query.total_records}}</span></h3>
            <code class="full-width text-center" v-show="!query.results" v-cloak>
                <div v-show="!query.fetching">Click "Search" to hit the Retsrabbit V2 API!</div>
                <div class="loader" v-show="query.fetching">
                    <div class="loader-inner ball-pulse-rise"></div>
                </div>
            </code>
            <code class="full-width code-md" v-show="query.results" class="text-center">{{ query.results }}</code>
        </div>
    </div>
</template>
